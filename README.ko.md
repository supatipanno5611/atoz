...
| 노트 정리 | 선택한 행을 원본별 Later 노트로 이동, 현재 파일 이동, 탭 정리 |
...
| `주제어 편집` | vault 노트와 제목, 기존 블록 ID를 검색해 주제어 위키링크를 추가·제거하거나 별칭을 변경합니다. |
...
리본 아이콘으로는 `작업 문서 열기`, `모바일 툴바 숨김 토글`, `Later 사이드바 열기`, 퀵 슬롯 열기를 바로 실행할 수 있습니다.
