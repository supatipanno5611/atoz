...
| 속성 관리 | `title`, `description`, `aliases`, `topics`, `date` 편집과 속성 정리 |
...
| 설정 | 기본값 | 설명 |
| --- | --- | --- |
| 블로그 폴더 | 비어 있음 | 규칙 기반 파일명과 순서를 적용할 폴더 및 카테고리입니다. |
...
리본 아이콘으로는 `작업 문서 열기`, `모든 폴더 숨김 토글`, `모바일 툴바 숨김 토글`, `클립보드 사이드바 열기`, `Later 사이드바 열기`, 퀵 슬롯 열기를 바로 실행할 수 있습니다.
