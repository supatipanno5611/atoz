...
글을 쓰는 동안 자주 반복하는 편집 동작, 작업 문서 정리, 게시용 frontmatter 준비, 주제어 관리, 유튜브/오디오 속성 입력, 조각글과 기호 입력을 명령어와 설정으로 묶어 둔 프로젝트입니다.
...
| 노트 정리 | 선택한 내용을 새 노트로 이동, 현재 파일을 다른 폴더로 이동, 특정 마크다운 파일 열기, 탭 정리 |
...
| 작업 문서 경로 | `work.md` | 작업 문서 명령에서 열고 정리할 파일입니다. |
| 보관 문서 경로 | `later.md` | 작업 문서 내용을 백업할 파일입니다. |
| 작업 시간 형식 | `MM/DD HH:mm:ss` | 작업 문서 백업 앞에 붙는 Moment.js 시간 형식입니다. |
...
1. `작업 문서 열기`로 `work.md`를 엽니다.
2. 임시 메모나 당장 처리할 내용을 적습니다.
3. `작업 문서 정리`를 실행합니다.
4. 작업 문서 내용이 타임스탬프와 함께 `later.md` 끝에 붙고, 작업 문서는 비워집니다.

작업 문서와 보관 문서 경로는 설정에서 바꿀 수 있습니다.
...
| `보관 문서 열기` | 설정한 보관 문서를 열고 마지막 heading으로 이동합니다. |
| `모든 탭 닫기` | 메인 작업 영역의 고정되지 않은 탭을 닫습니다. |
| `작업 문서 정리` | 작업 문서를 보관 문서에 백업하고 작업 문서를 비웁니다. |
| `현재 행을 파일로 이동` | 현재 행 또는 선택한 여러 행을 원본과 연결된 Later 노트로 이동합니다. |
...
리본 아이콘으로는 `작업 문서 열기`, `보관 문서 열기`, `프로젝트 폴더 숨김 토글`, `모바일 툴바 숨김 토글`, `클립보드 사이드바 열기`, `Later 사이드바 열기`를 바로 실행할 수 있습니다.
